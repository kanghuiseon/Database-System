...
int load_join_to_buffer_pool(){
    while(1){
        for(int i=0; i<buffer_size; i++){
            ...
            if(buffer_pool[i].is_dirty == true){
                if(buffer_pool[i].pagenum == 0){
                    file_write_header_page(buffer_pool[i].table_id, 0, &buffer_pool[i].header);
                }
                else{
                    file_write_page(buffer_pool[i].table_id, buffer_pool[i].pagenum, &buffer_pool[i].page);
                }
...
int join_table(int table_id_1, int table_id_2, char* pathname){
    if(tables[table_id_1-1].check == false || tables[table_id_2-1].check == false){
        printf("cannot find table!\n");
        return -1;
    }
    FILE* result_file;
    result_file = fopen(pathname, "wt");
    int num_of_join_key = 0;
    int result_index = load_join_to_buffer_pool();
    buffer_pool[result_index].is_pinned = true;
    int header_index_1 = get_page_index_from_buffer_pool(table_id_1, 0);
    int header_index_2 = get_page_index_from_buffer_pool(table_id_2, 0);
    
    pagenum_t root_offset_1 = buffer_pool[header_index_1].header.root_page_offset;
    if(root_offset_1 == 0){
        printf("There is not root!\n");
        return -1;
    }
    //start_key : leaf페이지 중 가장 왼쪽 페이지의 첫번째 키
    //leaf_index_first_1 : 가장 왼쪽의 leaf 페이지
    int leaf_index_first_1 = get_page_index_from_buffer_pool(table_id_1, root_offset_1);
    buffer_pool[leaf_index_first_1].is_pinned = true;
    while(!buffer_pool[leaf_index_first_1].page.is_Leaf){
        buffer_pool[leaf_index_first_1].is_pinned = false;
        leaf_index_first_1 = get_page_index_from_buffer_pool(table_id_1, buffer_pool[leaf_index_first_1].page.right_sibling_or_one_more_page_number);
        buffer_pool[leaf_index_first_1].is_pinned = true;
    }
    //start_key_1 테이블 1의 첫번째 leaf의 첫번째 key
    int64_t start_key_1 = buffer_pool[leaf_index_first_1].page.l_records[0].key;

    pagenum_t root_offset_2 = buffer_pool[header_index_2].header.root_page_offset;
    if(root_offset_2 == 0){
        printf("There is not root!\n");
        return -1;
    }
    int leaf_index_first_2 = get_page_index_from_buffer_pool(table_id_2, root_offset_2);
    buffer_pool[leaf_index_first_2].is_pinned = true;
    while(!buffer_pool[leaf_index_first_2].page.is_Leaf){
        buffer_pool[leaf_index_first_2].is_pinned = false;
        leaf_index_first_2 = get_page_index_from_buffer_pool(table_id_2, buffer_pool[leaf_index_first_2].page.right_sibling_or_one_more_page_number);
        buffer_pool[leaf_index_first_2].is_pinned = true;
    }
    //start_key_2 : 테이블 2의 첫번째 leaf의 첫번째 key
    int64_t start_key_2 = buffer_pool[leaf_index_first_2].page.l_records[0].key;
    int key_index_1 = 0, key_index_2 = 0;
    //테이블의 동일한 key값을 뽑아내서 파일에 쓴다.
    while(1){
        if((buffer_pool[leaf_index_first_1].page.right_sibling_or_one_more_page_number == 0 && key_index_1 == buffer_pool[leaf_index_first_1].page.num_keys-1 && buffer_pool[leaf_index_first_1].page.l_records[key_index_1].key <= start_key_2) || (buffer_pool[leaf_index_first_2].page.right_sibling_or_one_more_page_number == 0 && key_index_2 == buffer_pool[leaf_index_first_2].page.num_keys-1 &&  buffer_pool[leaf_index_first_2].page.l_records[key_index_2].key <= start_key_1)){
            file_write_result_join(result_index, result_file, num_of_join_key);
            break;
        }
        while(start_key_1 < start_key_2){
            if(key_index_1 == buffer_pool[leaf_index_first_1].page.num_keys-1){
                if(buffer_pool[leaf_index_first_1].page.right_sibling_or_one_more_page_number == 0)
                    break;
                //다음 leaf를 버퍼에 올린다.
                leaf_index_first_1 = get_page_index_from_buffer_pool(table_id_1, buffer_pool[leaf_index_first_1].page.right_sibling_or_one_more_page_number);
                key_index_1 = -1;
            }
            start_key_1 = buffer_pool[leaf_index_first_1].page.l_records[++key_index_1].key;
        }
        while(start_key_2 < start_key_1){
            if(key_index_2 == buffer_pool[leaf_index_first_2].page.num_keys-1){
                if(buffer_pool[leaf_index_first_2].page.right_sibling_or_one_more_page_number == 0)
                    break;
                leaf_index_first_2 = get_page_index_from_buffer_pool(table_id_2, buffer_pool[leaf_index_first_2].page.right_sibling_or_one_more_page_number);
                key_index_2 = -1;
            }
            start_key_2 = buffer_pool[leaf_index_first_2].page.l_records[++key_index_2].key;
        }
        //만약 두 테이블의 키값이 동일하면
        if(start_key_1 == start_key_2){
            if(num_of_join_key < 16){
                buffer_pool[result_index].join_result[num_of_join_key].key1 = start_key_1;
                strcpy(buffer_pool[result_index].join_result[num_of_join_key].value1, buffer_pool[leaf_index_first_1].page.l_records[key_index_1].value);
                buffer_pool[result_index].join_result[num_of_join_key].key2 = start_key_2;
                strcpy(buffer_pool[result_index].join_result[num_of_join_key].value2, buffer_pool[leaf_index_first_2].page.l_records[key_index_2].value);
                num_of_join_key++;
            }
            else if(num_of_join_key == 16){
                file_write_result_join(result_index, result_file, num_of_join_key);
                num_of_join_key = 0;
                buffer_pool[result_index].join_result[num_of_join_key].key1 = start_key_1;
                strcpy(buffer_pool[result_index].join_result[num_of_join_key].value1, buffer_pool[leaf_index_first_1].page.l_records[key_index_1].value);
    ...
    buffer_pool[result_index].table_id = 0;
    buffer_pool[result_index].is_pinned = false;
    buffer_pool[leaf_index_first_1].is_pinned = false;
    buffer_pool[leaf_index_first_2].is_pinned = false;
    fclose(result_file);
    return 0;
...
void file_write_result_join(int result_index, FILE* result_file, int num_of_join_key){
    int i=0;
    for(int i=0; i<num_of_join_key; i++){
        fprintf(result_file, "%lld,%s,%lld,%s\n",buffer_pool[result_index].join_result[i].key1, buffer_pool[result_index].join_result[i].value1, buffer_pool[result_index].join_result[i].key2, buffer_pool[result_index].join_result[i].value2);
        fflush(result_file);
    }
    return;
}
